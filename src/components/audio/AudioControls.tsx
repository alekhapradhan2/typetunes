...
export default function AudioControls({
  ...
  return (
    <div className="relative inline-flex items-center" ref={menuRef}>
      <div
        className="flex items-center gap-2 bg-slate-50/90 dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-2xl px-3 py-1.5 shadow-2xs"
      ...
      >
        ...
        <button
          id="audio-mute-btn"
          onClick={toggleMute}
          className="p-2 min-w-[36px] min-h-[36px] flex items-center justify-center rounded-lg text-slate-600 dark:text-slate-300 hover:text-sage-700 dark:hover:text-emerald-400 hover:bg-sage-100 dark:hover:bg-slate-700 transition-all flex-shrink-0 cursor-pointer"
        ...
        <input
          ...
          className="w-16 sm:w-20 h-1.5 appearance-none rounded-full cursor-pointer bg-slate-200 dark:bg-slate-700 accent-sage-600 dark:accent-emerald-500 focus:outline-none"
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-white dark:bg-slate-700 border border-slate-200/90 dark:border-slate-600 hover:bg-sage-50 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 hover:text-sage-800 dark:hover:text-white text-xs font-semibold transition-all shadow-2xs cursor-pointer"
        ...
        >
          ...
          <ChevronDown size={13} className={`text-slate-400 dark:text-slate-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
        </button>
      </div>
      ...
      {isOpen && (
        <div
          className={`absolute right-0 w-72 sm:w-80 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-2xl p-4 z-[100] animate-scale-in space-y-3.5 ${
            dropdownDirection === 'down' ? 'top-full mt-2.5' : 'bottom-full mb-2.5'
          }`}
        >
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
            <span className="text-xs font-bold text-slate-800 dark:text-white flex items-center gap-1.5">
              <Sliders size={14} className="text-sage-600 dark:text-emerald-400" />
              Keystroke Audio Engine
            </span>
            <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500">WebAudio 0ms</span>
          </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-medium transition-all text-left cursor-pointer ${
                      isSelected
                        ? 'bg-sage-50 dark:bg-emerald-950/80 text-sage-800 dark:text-emerald-300 border border-sage-300 dark:border-emerald-700 shadow-2xs font-bold'
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {isSelected && <Check size={14} className="text-sage-600 dark:text-emerald-400" />}
                    </div>
                  </button>
                );
              })}
              ...
              {SCALE_OPTIONS.map((scale) => {
                ...
                return (
                  <button
                    ...
                    className={`px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-all text-left truncate cursor-pointer ${
                      isSelected
                        ? 'bg-purple-100 dark:bg-purple-950/80 text-purple-800 dark:text-purple-300 border border-purple-300 dark:border-purple-700 font-bold'
                  ...
                  >
                    ...
                    <span className="text-[9px] text-slate-400 dark:text-slate-500">{scale.mood}</span>
                  </button>
                );
              })}
            </div>
