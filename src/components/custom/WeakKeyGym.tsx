    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/80 backdrop-blur-md p-5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <Target className="text-sage-600" size={22} />
        ...
        <div className="flex items-center gap-2 bg-sage-50 border border-sage-200 px-3.5 py-1.5 rounded-xl self-start sm:self-auto">
        ...
        <div className="flex flex-wrap gap-2">
          {PRESET_DRILLS.map((p) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-xl border transition-all hover:scale-102 ${p.color}`}
            >
              {p.label}
            </button>
          ))}
      ...
      <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400">Green = Active Target</span>
        </div>

        <div className="flex flex-col items-center gap-1.5 bg-slate-100/70 p-4 rounded-xl border border-slate-200/80 overflow-x-auto">
              ...
              {row.map((key) => {
                ...
                return (
                  <button
                    ...
                    className={`w-8 h-9 sm:w-10 sm:h-10 rounded-lg font-mono text-xs sm:text-sm font-semibold transition-all duration-150 flex items-center justify-center select-none shadow-2xs ${
                      isSelected
                        ? 'bg-sage-600 text-white scale-105 shadow-md shadow-sage-200 border-2 border-sage-700'
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-100">
          <div className="flex items-center gap-3">
            <span className="text-xs font-medium text-slate-600">Drill Length:</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Sparkles className="text-sage-600" size={18} />
            <h4 className="font-semibold text-sm text-slate-800">Generated Training Drill</h4>
          </div>
        ...
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 font-mono text-sm text-slate-700 leading-relaxed max-h-36 overflow-y-auto">
          {previewDrill}
        </div>
